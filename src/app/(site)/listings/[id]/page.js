...
export default function PropertyPage({ params }) {
  ...
  const [activeImage, setActiveImage] = useState(null)
  const [showAllPhotos, setShowAllPhotos] = useState(false)
  const [galleryStartIndex, setGalleryStartIndex] = useState(0) // NEW: Track which image to start gallery at
  const mapRef = useRef(null)
  const mapInstanceRef = useRef(null)
  ...
  // NEW: Function to open gallery at specific image
  const openGalleryAtImage = (imageIndex) => {
  ...
  const PhotoGallery = () => {
    const [currentIndex, setCurrentIndex] = useState(galleryStartIndex) // FIXED: Use galleryStartIndex instead of activeImage lookup
    ...
    return (
      <div className="relative">
        ...
        {showViewAllButton && (
          <button 
            type="button"
            onClick={() => openGalleryAtImage(0)} // UPDATED: Open at first image
            className="absolute top-4 right-4 z-10 bg-white rounded-full px-4 py-2 flex items-center text-sm font-medium shadow-md hover:bg-gray-100 transition-colors"
          >
            <Camera size={16} className="mr-2" />
            View all photos
        ...
        <div className="hidden md:block">
          {/* Single image layout */}
          {imageCount === 1 && (
            <div className="h-[60vh] relative rounded-xl overflow-hidden cursor-pointer" onClick={() => openGalleryAtImage(0)}>
              {renderImage(allImages[0], property?.title || "Property image", "rounded-xl", true)}
            </div>
          )}
          ...
          {imageCount === 2 && (
            <div className="grid grid-cols-2 gap-4 h-[60vh]">
              <div className="relative rounded-l-xl overflow-hidden cursor-pointer" onClick={() => openGalleryAtImage(0)}>
                {renderImage(allImages[0], property?.title || "Property image", "rounded-l-xl", true)}
              </div>
              <div className="relative rounded-r-xl overflow-hidden cursor-pointer" onClick={() => openGalleryAtImage(1)}>
                {renderImage(allImages[1], `${property?.title || "Property"} side view`, "rounded-r-xl")}
              </div>
            </div>
          )}
          ...
          {imageCount === 3 && (
            <div className="grid grid-cols-2 gap-4 h-[60vh]">
              ...
              <div className="grid grid-rows-2 gap-4">
                <div className="relative rounded-tr-xl overflow-hidden cursor-pointer" onClick={() => openGalleryAtImage(1)}>
                  {renderImage(allImages[1], `${property?.title || "Property"} side view 1`, "rounded-tr-xl")}
                </div>
                <div className="relative rounded-br-xl overflow-hidden cursor-pointer" onClick={() => openGalleryAtImage(2)}>
                  {renderImage(allImages[2], `${property?.title || "Property"} side view 2`, "rounded-br-xl")}
                </div>
              </div>
            </div>
          ...
          {imageCount === 4 && (
            <div className="grid grid-cols-2 gap-4 h-[60vh]">
              ...
              <div className="grid grid-rows-3 gap-4">
                ...
                <div className="relative overflow-hidden cursor-pointer" onClick={() => openGalleryAtImage(2)}>
                  {renderImage(allImages[2], `${property?.title || "Property"} side view 2`)}
                </div>
                <div className="relative rounded-br-xl overflow-hidden cursor-pointer" onClick={() => openGalleryAtImage(3)}>
                  {renderImage(allImages[3], `${property?.title || "Property"} side view 3`, "rounded-br-xl")}
                </div>
              </div>
            </div>
          ...
          {imageCount >= 5 && (
            <div className="grid grid-cols-4 grid-rows-2 gap-4 h-[60vh]">
              {/* Main large image (left column) */}
              <div className="col-span-2 row-span-2 relative rounded-l-xl overflow-hidden cursor-pointer" onClick={() => openGalleryAtImage(0)}>
                {renderImage(allImages[0], property?.title || "Property image", "rounded-l-xl", true)}
              </div>
              
              {/* Top right images */}
              <div className="relative overflow-hidden cursor-pointer" onClick={() => openGalleryAtImage(1)}>
                {renderImage(allImages[1], `${property?.title || "Property"} side view 1`)}
              </div>
              
              <div className="relative overflow-hidden cursor-pointer rounded-tr-xl" onClick={() => openGalleryAtImage(2)}>
                {renderImage(allImages[2], `${property?.title || "Property"} side view 2`, "rounded-tr-xl")}
              </div>
              
              {/* Bottom right images */}
              <div className="relative overflow-hidden cursor-pointer" onClick={() => openGalleryAtImage(3)}>
                {renderImage(allImages[3], `${property?.title || "Property"} extra view 1`)}
              </div>
              
              <div className="relative overflow-hidden cursor-pointer rounded-br-xl" onClick={() => openGalleryAtImage(4)}>
                <div className="relative w-full h-full">
                  {renderImage(allImages[4], `${property?.title || "Property"} extra view 2`, "rounded-br-xl")}
                  
                  {/* Only show the overlay if there are more than 5 images */}
                  {imageCount > 5 && (
                    <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50 text-white font-medium rounded-br-xl cursor-pointer">
                      +{allImages.length - 5} more
                    </div>
                  )}
                </div>
        ...
        {/* Mobile Layout - UPDATED with specific image clicks */}
        <div className="md:hidden">
          {/* Main image */}
          <div className="relative w-full h-64 rounded-t-xl overflow-hidden cursor-pointer" onClick={() => openGalleryAtImage(0)}>
            {renderImage(allImages[0], property?.title || "Property image", "rounded-t-xl", true)}
          </div>
            ...
            <div className="flex overflow-x-auto space-x-2 pb-2 mt-2">
              {allImages.slice(1).map((img, idx) => (
                <div 
                  key={idx}
                  className="relative w-28 h-28 flex-shrink-0 cursor-pointer"
                  onClick={() => openGalleryAtImage(idx + 1)} // UPDATED: Open at specific image (offset by 1 since we're slicing from index 1)
                >
                  <Image
                    src={img}
                    alt={`${property?.title || "Property"} view ${idx + 2}`}
